...
class Grid_50_tmp(Base):
    __tablename__ = 'grid_50_tmp'
    id = Column(Integer, primary_key=True)
    geometry = Column(geoalchemy2.Geometry('POLYGON', srid=4326))
...
def create_and_insert_grid_into_db(cell_size_km):
    ...
    try:
        for _, row in grid_gdf.iterrows():
            ...
            if cell_size_km == 0.8:
                grid_entry = Grid_800_tmp(geometry=geom)
            if cell_size_km == 0.4:
                grid_entry = Grid_400_tmp(geometry=geom)
            if cell_size_km == 0.2:
                grid_entry = Grid_200_tmp(geometry=geom)
            if cell_size_km == 0.1:
                grid_entry = Grid_100_tmp(geometry=geom)
            if cell_size_km == 0.05:
                grid_entry = Grid_50_tmp(geometry=geom)
...
def create_and_populate_grid_800(engine):
    # SQL command to create the table and insert data
    sql_command = text("""
    INSERT INTO public.grid_800 (id, geometry, avg_depth)
...
def create_and_populate_grid_200(engine):
    # SQL command to create the table and insert data
    sql_command = text("""
    INSERT INTO public.grid_200 (id, geometry, avg_depth)
...
def create_and_populate_grid_100(engine):
    # SQL command to create the table and insert data
    sql_command = text("""
    INSERT INTO public.grid_100 (id, geometry, avg_depth)
...
def create_and_populate_grid_50(engine):
    # SQL command to create the table and insert data
    sql_command = text("""
    INSERT INTO public.grid_50 (id, geometry, avg_depth)
...
create_and_insert_grid_into_db(0.05)
create_and_populate_grid_50(engine)   
