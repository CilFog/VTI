...
def add_nodes_and_edges(G, trajectory_points, edge_dist_threshold):
    ...
    return G, added_nodes, added_edges, execution_time, tree
...
def find_and_impute_paths(G, trajectory_points, file_name, node_dist_threshold, edge_dist_threshold, cog_angle_threshold, type, size, added_nodes, added_edges, add_execution_time, tree):
    start_time = time.time()
    ...
    for i in range(len(trajectory_points) - 1):
        ...
        if direct_path_exists:
            ...
            try:
                #G = adjust_edge_weights_for_draught(G, start_point, end_point, tree)
...
def load_graphs_and_impute_trajectory(file_name, file_path, G, node_dist_threshold, edge_dist_threshold, cog_angle_threshold, type, size):
    ...
    new_g, added_nodes, added_edges, add_execution_time, tree = add_nodes_and_edges(G, trajectory_points, edge_dist_threshold)
    find_and_impute_paths(new_g, trajectory_points, file_name, node_dist_threshold, edge_dist_threshold, cog_angle_threshold, type, size, added_nodes, added_edges, add_execution_time, tree)
